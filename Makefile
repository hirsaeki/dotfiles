...
CONDA_PKGS := git fish tmux powerline-status jq yq tree unzip patch fzf openshift-cli kubernetes-client consul
FISH_PLUGINS := danhper/fish-ssh-agent oh-my-fish/theme-agnoster jethrokuan/fzf 2m/fish-history-merge 0rax/fish-bd
TFENVS := 
...
conda-base: ## initialize base env via miniconda+conda-forge with essential package. e.g. fish, tmux
	...
	@eval "$$($(HOME)/miniconda/bin/conda shell.bash hook)" && \
		...
		$(foreach val, $(FISH_PLUGINS), grep -q $(val) .config/fish/fish_plugins || echo "$(val)" >> .config/fish/fish_plugins;) \
		/usr/bin/env fish -c "fisher update"; \
